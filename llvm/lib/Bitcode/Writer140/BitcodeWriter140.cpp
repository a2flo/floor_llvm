//===- Bitcode/Writer140/BitcodeWriter140.cpp - Bitcode 14.0 Writer -------===//
//
...
//===----------------------------------------------------------------------===//
//
// Bitcode 14.0 writer implementation.
//
//===----------------------------------------------------------------------===//

// enable errors when using > 14.0 bitcode enums from LLVMBitCodes.h
#define LLVM_BITCODE_140 1
...
static cl::opt<unsigned>
    IndexThreshold("bitcode-mdindex-threshold-bc140", cl::Hidden, cl::init(25),
                   cl::desc("Number of metadatas above which we emit an index "
                            "to enable lazy-loading"));
static cl::opt<uint32_t> FlushThreshold(
    "bitcode-flush-threshold-bc140", cl::Hidden, cl::init(512),
    cl::desc("The threshold (unit M) for flushing LLVM bitcode."));

static cl::opt<bool> WriteRelBFToSummary(
    "write-relbf-to-summary-bc140", cl::Hidden, cl::init(false),
...
void llvm::WriteBitcodeToFile140(const Module &M, raw_ostream &Out,
                                 bool ShouldPreserveUseListOrder,
                                 const ModuleSummaryIndex *Index,
                                 bool GenerateHash, ModuleHash *ModHash) {
...
class ThinLinkBitcodeWriter140 : public ModuleBitcodeWriterBase140 {
  /// ModHash is for use in ThinLTO incremental build, generated while writing
  /// the module bitcode file.
  const ModuleHash *ModHash;

public:
  ThinLinkBitcodeWriter140(const Module &M, StringTableBuilder &StrtabBuilder,
                        BitstreamWriter &Stream,
                        const ModuleSummaryIndex &Index,
                        const ModuleHash &ModHash)
      : ModuleBitcodeWriterBase140(M, StrtabBuilder, Stream,
...
// as padding.
void ThinLinkBitcodeWriter140::writeSimplifiedModuleInfo() {
  SmallVector<unsigned, 64> Vals;
  // Emit the module's source file name.
  {
    StringEncoding Bits = getStringEncoding(M.getSourceFileName());
...
void ThinLinkBitcodeWriter140::write() {
  Stream.EnterSubblock(bitc::MODULE_BLOCK_ID, 3);
...
void BitcodeWriter140::writeThinLinkBitcode(const Module &M,
  ...
  ThinLinkBitcodeWriter140 ThinLinkWriter(M, StrtabBuilder, *Stream, Index,
                                       ModHash);
...
void llvm::writeThinLinkBitcodeToFile140(const Module &M, raw_ostream &Out,
                                         const ModuleSummaryIndex &Index,
                                         const ModuleHash &ModHash) {
  SmallVector<char, 0> Buffer;
  Buffer.reserve(256 * 1024);
...
void llvm::embedBitcodeInModule140(llvm::Module &M, llvm::MemoryBufferRef Buf,
  ...
  if (EmbedBitcode) {
    if (Buf.getBufferSize() == 0 ||
      ...
      llvm::WriteBitcodeToFile140(M, OS, /* ShouldPreserveUseListOrder */ true);
      ModuleData =
