...
void CodeGenTypes::create_flattened_cg_layout(const CXXRecordDecl* D, llvm::StructType* Ty,
	...
	const auto is_unnamed = !Ty->hasName();
	...
	for (const auto& field : fields) {
		...
		for (const auto& parent : field.parents) {
			if (parent != D) { // only add actual parents, not D itself
	...
	// retrieve or add an entry for this decl
	auto flat_rec_iter = flattened_records.find(D);
	if (flat_rec_iter == flattened_records.end()) {
		bool did_insert = false;
		std::tie(flat_rec_iter, did_insert) = flattened_records.insert({ D, flattened_record_t {} });
		assert(flat_rec_iter != flattened_records.end() && did_insert);
	}
	
	// update entry, assert this is only done once
	auto& flat_rec = flat_rec_iter->second;
	if (is_floor_arg_buffer) {
		assert(!flat_rec.arg_buffer.type);
		flat_rec.arg_buffer = { Ty, RL };
	} else if (!is_unnamed) {
		assert(!flat_rec.named.type);
		flat_rec.named = { Ty, RL };
	} else {
		assert(!flat_rec.unnamed.type);
		flat_rec.unnamed = { Ty, RL };
	}
	
	// for all parents, add a link to this decl + create initial entries where necessary
	for (const auto& parent_decl : parent_decls) {
		auto parent_rec_iter = flattened_records.find(parent_decl);
		if (parent_rec_iter == flattened_records.end()) {
			bool did_insert = false;
			std::tie(parent_rec_iter, did_insert) = flattened_records.insert({ parent_decl, flattened_record_t {} });
			assert(parent_rec_iter != flattened_records.end() && did_insert);
		}
		parent_rec_iter->second.children.insert(D);
	}
	
	// for all bases, add an "allowed" alias for this decl
	for (const auto& base_alias_decl : base_alias_decls) {
		assert(flat_rec.aliases.count(base_alias_decl) == 0); // must never form a ring
		auto base_rec_iter = flattened_records.find(base_alias_decl);
		if (base_rec_iter == flattened_records.end()) {
			bool did_insert = false;
			std::tie(base_rec_iter, did_insert) = flattened_records.insert({ base_alias_decl, flattened_record_t {} });
			assert(base_rec_iter != flattened_records.end() && did_insert);
		}
		base_rec_iter->second.aliases.insert(D);
	}
	
#ifndef NDEBUG
	if (!is_unnamed) {
		if (is_floor_arg_buffer) {
			should_have_flattened_layout_arg_buf.insert({ Ty });
		} else {
